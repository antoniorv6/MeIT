...
def main():
    train_set, val_set, test_set = load_data(data_path="Data/string_quartets_dataset/partitions_strquartets/excerpts/fold_0", vocab_name="quartets")
    ...
    model = get_DAN_MeIT(maxheight, maxwidth, maxlen, "weights/MiT_MAE_B-epoch=6-step=1700000-val_loss=0.06149.ckpt", 
                         out_categories=len(w2i), padding_token=w2i['<pad>'], w2i=w2i, i2w=i2w)
    
    wandb_logger = WandbLogger(project='LITIS_STAY', group=f"SSL", name=f"MeIT_MAE_SQ", log_model=False)

    early_stopping = EarlyStopping(monitor='val_SER', min_delta=0.01, patience=5, mode="min", verbose=True)
    
    checkpointer = ModelCheckpoint(dirpath=f"weights/Quartets/", filename=f"DAN_MEiT_MAE", 
                                   monitor='val_SER', mode='min',
                                   save_top_k=1, verbose=True)

    trainer = Trainer(max_epochs=10000, check_val_every_n_epoch=5, logger=wandb_logger, callbacks=[checkpointer, early_stopping])
